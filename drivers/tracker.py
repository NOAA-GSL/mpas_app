...
class GFDLTracker(DriverCycleBased):
    ...
    def input_vitals_other_names(self):
        ...
        yield [Asset(path, path.exists) for path in paths]
        yield self.input_vitals()
        for path in paths:
            path.symlink_to(self.rundir / "allvit")
    ...
    def namelist_file(self):
        ...
        update_values.setdefault("datein", {}).update(
            {
                "inp": {
                    "bcc": int(self.cycle.strftime("%Y")[0:2]),
                    "byy": int(self.cycle.strftime("%y")),
                    "bmm": int(self.cycle.strftime("%m")),
                    "bdd": int(self.cycle.strftime("%d")),
                    "bhh": int(self.cycle.strftime("%H")),
                }
            }
        )
        update_values.setdefault("atcfinfo", {}).update(
            {
